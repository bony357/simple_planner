...
import { scheduleTasksSync } from '../services/google/tasksSync'
...
export async function deleteTask(id: string): Promise<void> {
  await db.transaction('rw', db.tasks, db.events, async () => {
    ...
    if (task?.googleTaskId) {
      // Oznacz do usunięcia w Google Tasks przy najbliższym sync.
      await db.tasks.update(id, { syncState: 'deleted' })
    } else {
